...
export class KafkaProducerService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly configService: ConfigService) {
    this.kafka = new Kafka({
      clientId: 'daltaners-auth-service',
  ...
  async publish<T>(topic: string, eventType: string, data: T, key?: string): Promise<void> {
    const event: DaltanersEvent<T> = {
      specversion: '1.0',
      id: uuidv4(),
      source: 'daltaners/auth-service',
      type: `com.daltaners.users.${eventType}`,
      datacontenttype: 'application/json',
    ...
    try {
      ...
      // Don't throw — user registration should not fail because of Kafka
    }
  }
}
